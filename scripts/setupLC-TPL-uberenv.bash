...
function launch_jobs() {
  ...
  case "$machine" in
    dane)
      ...
      "${UBERENV_HELPER}" "$INSTALL_DIR" dane gcc-12  "+docs +hypredrive %%gcc-12 ${COMMON}"   "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" dane gcc-13  "+docs +hypredrive %%gcc-13 ${COMMON}"   "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" dane llvm-14 "+docs +hypredrive %%clang-14 ${COMMON}" "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" dane llvm-19 "+docs +hypredrive %%clang-19 ${COMMON}" "${ALLOC_CMD}" "$@" &
      ;;

    matrix)
      ALLOC_CMD="srun -N 1 --exclusive -t 60 -A vortex"
      "${UBERENV_HELPER}" "$INSTALL_DIR" matrix gcc-12-cuda-12.6  "+cuda ~uncrustify +hypredrive cuda_arch=90 %%gcc-12 ^cuda@12.6.0+allow-unsupported-compilers ${COMMON}"   "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" matrix gcc-13-cuda-12.9  "+cuda ~uncrustify +hypredrive cuda_arch=90 %%gcc-13 ^cuda@12.9.1+allow-unsupported-compilers ${COMMON}"   "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" matrix llvm-14-cuda-12.6 "+cuda ~uncrustify +hypredrive cuda_arch=90 %%clang-14 ^cuda@12.6.0+allow-unsupported-compilers ${COMMON}" "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" matrix llvm-19-cuda-12.9 "+cuda ~uncrustify +hypredrive cuda_arch=90 %%clang-19 ^cuda@12.9.1+allow-unsupported-compilers ${COMMON}" "${ALLOC_CMD}" "$@" &
      ;;

    tuo|tuolumne)
      ALLOC_CMD="salloc -N 1 --exclusive -p pdebug -t 60 -A vortex"
      "${UBERENV_HELPER}" "$INSTALL_DIR" tuolumne cce-20-rocm-6.4.3  "+rocm~pygeosx~trilinos~petsc~docs+hypredrive amdgpu_target=gfx942 %%cce-20 ${COMMON}" "${ALLOC_CMD}" "$@" &
      "${UBERENV_HELPER}" "$INSTALL_DIR" tuolumne llvm-amdgpu-6.4.3  "+rocm~pygeosx~trilinos~petsc~docs+hypredrive amdgpu_target=gfx942 %%llvm-amdgpu_6_4_3 ${COMMON}" "${ALLOC_CMD}" "$@" &
      ;;
