...
class Geosx(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant('hypre-unified-memory', default=True, description='Enable HYPRE unified memory for GPU builds.')
    ...
    with when("+hypre"):
        ...
        depends_on("hypre +cuda+superlu-dist+mixedint+mpi+umpire+unified-memory", when='+cuda+hypre-unified-memory')
        depends_on("hypre +cuda+superlu-dist+mixedint+mpi+umpire~unified-memory", when='+cuda~hypre-unified-memory')
        depends_on("hypre +rocm+superlu-dist+mixedint+mpi+umpire+unified-memory", when='+rocm+hypre-unified-memory')
        depends_on("hypre +rocm+superlu-dist+mixedint+mpi+umpire~unified-memory", when='+rocm~hypre-unified-memory')
